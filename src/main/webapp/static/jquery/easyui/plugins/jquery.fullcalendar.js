...
(function ($) {
	...
	function showMonth(target, delta) {
		var opts = $.data(target, 'fullCalendar').options;
		opts.month = parseInt(opts.month) + parseInt(delta);
		if (opts.month > 12) {
			opts.year++;
			opts.month = 1;
	...
	function showYear(target, delta) {
		var opts = $.data(target, 'fullCalendar').options;
		opts.year = parseInt(opts.year)+parseInt(delta);
		show(target);
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				...
				if (opts.month == day[1]) {
					...
					dayHtml += '<div style="display:none;color:red;">节假日</div>';
				}
				var day = $('<td class="calendar-day fullcalendar-day calendar-other-month" onClick="ShowDialog(this)"></td>').data('info', info).attr('abbr', day[0] + ',' + day[1] + ',' + day[2]).html(dayHtml).appendTo(tr);
