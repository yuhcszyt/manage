...
//    };
    ...
    $.fn.euploadify.defaults = {
            ...
            */
            formData: {},
            /**
             * 表示上传文件队列的 HTML 页面显示模板；该值定义为 false 表示按照默认格式显示；
             * 该模版中可以使用如下四个变量标签：
             * instanceID:
             * fileID:
             * fileName:
            ...
            */
            ...
            queueID: false,
			/**
			 * 表示能够同时执行上传操作的文件最大数量；这并不限制该控件可上传文件的最大数量；
			 * 要限制可上传文件最大数量请用 uploadLimit 参数；
    ...
    $.extend($.fn.euploadify.defaults, {
        ...
        /**
         * 设置按钮提示文字；
         * 属性可以为一个格式如 { button: string, upload: string, stop: string, cancel: string, remove: string } 的对象，表示每个按钮的提示文字内容；
        ...
        */
        ...
        responseFilter: function (data, file) {
            return utils.tryExec(function () { return utils.parseJSON(data); });
        },
    ...
    function create(target) {
        var t = $(target).addClass("euploadify-f").hide(),
            state = $.data(target, "euploadify"), 
            opts = state.options,
            name = state.name = t.attr("name");
        state.uploadifyID = "euploadify_" + utils.guid("N");
        state.queueID = "euploadify_queue_" + utils.guid("N");
    ...
    */
    ...
    function setValues(target, values) {
        var t = $(target), state = $.data(target, "euploadify"), opts = state.options,
            ...
            array = utils.likeArrayNotString(values) ? values : [values];
    ...
    (function () {
        ...
        if ($.inArray("euploadify", plugins) == -1) {
